...
func main() {
	...
	N := getI()
	a := getInts(N)
	// a := []int{44000302, 31035145, 55195208, 44312030, 41832822, 60638742, 72118265, 72118265, 89210757}
	// a := []int{1, 1, 3}
	// N := len(a)
	// ret := f(a)
	// out(ret)
	...
	v := make([]S, len(m)+1)
	e := func() S { return S{0, 0} }
	merger := func(a, b S) S {
		return S{a.val + b.val, a.cnt + b.cnt}
	}
	...
	for i := 0; i < N; i++ {
		...
		y := seg.Get(x)
		// out(y)
		seg.Set(x, S{a[i] + y.val, y.cnt + 1})
		e := seg.Prod(0, x)
		ans += a[i]*e.cnt - e.val
		// out(a[i], e, a[i]*e.cnt, e.val)
	}
