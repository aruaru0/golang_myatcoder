...
func f() {
	...
	segL := SegtreeInit(N+1, 0)
	segR := SegtreeInit(N+1, 0)
	...
	for i := 0; i < N; i++ {
		if a[i].l >= a[i].r {
			pos := int(segL.Query(1, a[i].k+1))
			if pos > 0 {
				ans += a[i].l
				segL.UpdateAt(pos, Data(-1))
			} else {
				// Ｌ側に置ける場所がないとき
				ans += a[i].r
			}
		} else {
			pos := int(segR.Query(1, N-a[i].k+1))
			if pos > 0 {
				ans += a[i].r
				segR.UpdateAt(pos, Data(-1))
			} else {
				// Ｒ側に置ける場所がないとき
				ans += a[i].l
			}
