...
func main() {
	...
	for i := 1; i <= m; i++ {
		...
		p := mod.div(mod.sub(m+1, i), m)
		p_pow := make([]int, zero+1)
		q_pow := make([]int, zero+1)
		for j := 0; j < zero+1; j++ {
			p_pow[j] = 1
			q_pow[j] = 1
		}
		for j := 0; j < zero; j++ {
			p_pow[j+1] = mod.mul(p_pow[j], p)
			q_pow[j+1] = mod.mul(q_pow[j], mod.sub(1, p))
		}
		for j := rem; j <= zero; j++ {
			tmp := mod.mul(mod.nCr(zero, j), mod.mul(p_pow[j], q_pow[zero-j]))
			ans = mod.add(ans, tmp)
		}
	}
	out(ans)
