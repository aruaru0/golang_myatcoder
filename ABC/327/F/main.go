...
func main() {
	...
	e := func() S { return S{0} }
	merger := func(a, b S) S { return S{max(a.x, b.x)} }
	mapper := func(f F, x S) S { return S{f.x + x.x} }
	...
	for i := 0; i < n; i++ {
		t, x := getI(), getI()
		tmax := max(t-d, 0)
		q[tmax] = append(q[tmax], pair{x, 1})
		q[t] = append(q[t], pair{x, -1})
	}
	...
	for i := 0; i < Tmax; i++ {
		sz := len(q[i])
		for j := 0; j < sz; j++ {
			seg.RangeApply(max(0, q[i][j].x-w), q[i][j].x, F{q[i][j].t})
		}
