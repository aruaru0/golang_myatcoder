...
func main() {
	...
	bars := make([]BAR, N)
	for i := 0; i < N; i++ {
		bars[i].r, bars[i].c, bars[i].l = getI(), getI(), getI()
		bars[i].idx = i
	}

	sort.Slice(bars, func(i, j int) bool {
		if bars[i].r == bars[j].r {
			return bars[i].c < bars[j].c
		}
		return bars[i].r > bars[j].r
	})
	...
	merger := func(a, b S) S {
		return S{max(a.a, b.a)}
	}
	mapper := func(f F, x S) S {
		return S{max(f.a, x.a)}
	}
	comp := func(f, g F) F {
		return F{max(f.a, g.a)}
	}
	id := func() F {
		return F{0}
	}

	seg := newLazySegtree(make([]S, W+1), e, merger, mapper, comp, id)

	ans := make([]int, N)
	for i := 0; i < N; i++ {
		bar := bars[i]
		pos := seg.Prod(bar.c, bar.c+bar.l)
		ans[bar.idx] = H - pos.a
		seg.RangeApply(bar.c, bar.c+bar.l, F{pos.a + 1})
	}
