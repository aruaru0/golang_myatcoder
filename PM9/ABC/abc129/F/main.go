...
func main() {
	...
	L, A, B, M := getI(), getI(), getI(), getI()

	mod := newModint(M)
	x := 1
	cnt := 0

	y := [][]int{[]int{1, 0, 0}, []int{0, 1, 0}, []int{0, 0, 1}}
	...
	for i := 0; i < 18; i++ {
		...
		a := [][]int{[]int{x % M, 0, 0}, []int{1, 1, 0}, []int{0, B % M, 1}}
		a = mod.powModMatrix(a, num)
		y = mod.mulMod(y, a)
		// out(i, x, num, cnt, y)
	}
	ans := (mod.mul(y[0][0], 0) + mod.mul(y[1][0], A%M) + y[2][0]) % M
	out(ans)
}
