...
func main() {
	...
	for i := 0; i < n; i++ {
		l, r := getI(), getI()
		rects = append(rects, Rect{0, l, l + 1, r})
		rects = append(rects, Rect{l + 1, r, r + 1, M + 1})
	}
	...
	xr, yr := make(map[int]int), make(map[int]int)
	for i, e := range tx {
		xs[e] = i
		xr[i] = e
	}
	...
	//---
	events := make([][]Event, len(xs))
	for _, e := range rects {
		lx := xs[e.lx]
		rx := xs[e.rx]
		ly := ys[e.ly]
		ry := ys[e.ry]
		events[lx] = append(events[lx], Event{ly, ry, 1})
		events[rx] = append(events[rx], Event{ly, ry, -1})
	}
	...
	t := newLazySegtree(v, e, merger, mapper, comp, id)

	ans, ansl, ansr := 0, 0, 1
	for xi := 0; xi < len(xs); xi++ {
		for _, e := range events[xi] {
			t.RangeApply(e.l, e.r, F{e.val})
		}
		s := t.AllProd()
		// out(s)
		if ans < s.val {
			ans = s.val
			ansl, ansr = xr[xi], yr[s.y]
		}
	}
	// out(xs)
	out(ansl, ansr)
