...
func dfs(cur, prev, cost int) {
	dist[cur] = cost
	for _, e := range node[cur] {
		if e.to == prev {
			continue
		}
		dfs(e.to, cur, cost^e.cost)
	}
}
...
func main() {
	...
	for i := 0; i < 60; i++ {
		one := c[i]
		zero := N - c[i]
		// １になるのは0と1の組なので、１になる組み合わせ数を数える
		sum := mod.mul(one, zero) // 1の数 * 0の数
		// 各桁のsumを加算していく（２のn乗の部分に）
		tot = mod.add(tot, mod.mul(d, sum))
		d = mod.mul(d, 2)
		// out(one, zero, sum)
	}
	out(tot)
