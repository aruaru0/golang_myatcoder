...
func main() {
	...
	for i := 0; i < N; i++ {
		t := getI() % mod.mod
		a[i] = S{t, mod.mul(t, i), mod.mul(t, mod.mul(i, i))}
	}

	e := func() S { return S{0, 0, 0} }
	merger := func(a, b S) S {
		return S{mod.add(a.x, b.x), mod.add(a.y, b.y), mod.add(a.z, b.z)}
	}
	...
	for i := 0; i < Q; i++ {
		...
		if t == 1 {
			x, y := getI()-1, getI()
			seg.Set(x, S{y, mod.mul(y, x), mod.mul(y, mod.mul(x, x))})
		} else {
			x := getI() - 1
			ret := seg.Prod(0, x+1)
			a := mod.div(ret.z, 2)
			b := mod.div(mod.mul(ret.y, 2*x+3), 2)
			c := mod.div(mod.mul(ret.x, mod.mul(x+1, x+2)), 2)
			out(mod.add(mod.sub(a, b), c))
		}
	}
}
