...
func main() {
	...
	v0 := make([]int, N)
	e0 := func() int { return inf }
	merger0 := func(a, b int) int { return min(a, b) }
	for i := 0; i < N; i++ {
		v0[i] = inf
	}

	v1 := make([]int, N)
	e1 := func() int { return -inf }
	merger1 := func(a, b int) int { return max(a, b) }
	for i := 0; i < N; i++ {
		v1[i] = -inf
	}

	seg_min := newSegtree(v0, e0, merger0)
	seg_max := newSegtree(v1, e1, merger1)
	...
	min_val := seg_min.Prod(0, N)
	max_val := seg_max.Prod(0, N)
	ans := max_val - min_val
