...
func main() {
	...
	dpL[0][0] = 1
	for i := 0; i < N-1; i++ {
		dpL[i+1][0] = mod.add(dpL[i][0], dpL[i][1])
		dpL[i+1][1] = dpL[i][0] // -の連続はNG
	}
	dpR := make([][2]int, N)
	dpR[N-1][0] = 1
	for i := N - 1; i > 0; i-- {
		dpR[i-1][0] = mod.add(dpR[i][0], dpR[i][1])
		dpR[i-1][1] = dpR[i][0]
	}

	ans := mod.mul(a[0], mod.add(dpR[0][0], dpR[0][1]))
	for i := 1; i < N; i++ {
		tot := mod.mul(a[i], mod.mul(mod.add(dpL[i-1][0], dpL[i-1][1]), mod.add(dpR[i][0], dpR[i][1])))
		tot = mod.sub(tot, mod.mul(a[i], mod.mul(dpL[i][1], dpR[i-1][1])))
		ans = mod.add(ans, tot)
	}
	out(ans)
}
