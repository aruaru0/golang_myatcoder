...
func main() {
	...
	// DPテーブルの初期化
	// S : 一度も移動させない人の順序付き集合
	// dp[i] :  Sに含まれる人の番号の最大値が i である場合において、番号が i 以下の人々を昇順に並び替えるのにかかる合計コストの最小値)
	dp := make([]int, N+1)
	for i := 0; i <= N; i++ {
		dp[i] = inf
	}
	...
	e := func() int { return inf }
	merger := func(a, b int) int { return min(a, b) }
