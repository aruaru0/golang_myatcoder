...
func solve() {
	N, A1, B1, A2, B2, A3, B3 := getI(), getI(), getI(), getI(), getI(), getI(), getI()

	mod := newModint(1e9 + 7)
	p1 := mod.div(A1, B1)
	p2 := mod.div(A2, B2)
	p3 := mod.div(A3, B3)
	x0 := mod.sub(mod.pow(mod.add(p1, p2), N), mod.add(mod.pow(p1, N), mod.pow(p2, N)))
	x1 := mod.sub(mod.pow(mod.add(p1, p3), N), mod.add(mod.pow(p1, N), mod.pow(p3, N)))
	x2 := mod.sub(mod.pow(mod.add(p2, p3), N), mod.add(mod.pow(p2, N), mod.pow(p3, N)))
	x := mod.sub(1, x0)
	x = mod.sub(x, x1)
	x = mod.sub(x, x2)
	out(x)
