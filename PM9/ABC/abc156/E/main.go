...
func main() {
	...
	ans := 0
	// iは人のいない部屋の個数（最大Kかn-1）
	for i := 0; i <= min(k, n-1); i++ {
		// nCr(n, i) : 人のいない部屋の組み合わせ
		x := mod.nCr(n, i)
		t := n - i                 // 人の以内部屋の数を引いた部屋の数
		y := mod.nCr(t+n-t-1, n-t) // 人のいない部屋以外に、人を配置（１人は必ず配置する必要があるので、n-t人が自由に配置できる人数）
		ans = mod.add(ans, mod.mul(x, y))
	}
	out(ans)
}
