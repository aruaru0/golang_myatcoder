...
func main() {
	...
	e := func() int { return 0 }
	merger := func(a, b int) int { return gcd(a, b) }
	v := make([]int, N)
	ta := newSegtree(v, e, merger)
	tb := newSegtree(v, e, merger)
	for i := 0; i < N-1; i++ {
		ta.Set(i, abs(a[i+1]-a[i]))
		tb.Set(i, abs(b[i+1]-b[i]))
	}

	for q := 0; q < Q; q++ {
		h1, h2, w1, w2 := getI()-1, getI(), getI()-1, getI()
		ans := a[h1] + b[w1]
