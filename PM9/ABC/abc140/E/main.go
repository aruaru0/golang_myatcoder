...
func main() {
	...
	merger2 := func(a, b int) int {
		return min(a, b)
	}
	segR := newSegtree(v2, e2, merger2)
	...
	for i := 0; i < N; i++ {
		...
		l := segL.Prod(0, idx)
		r := segR.Prod(idx, N+3)
		ll := max(0, segL.Prod(0, l))
		rr := segR.Prod(r+1, N+3)
		dl, dr := max(0, idx-l), max(0, r-idx)
		dll, drr := max(0, l-ll), max(0, rr-r)
		// out(idx, p[i], "LR", l, r, "LLRR", ll, rr, "d", dl, dr, dll, drr, "tot", dl*drr+dr*dll, (dl*drr+dr*dll)*p[i].v)
		ans += (dl*drr + dr*dll) * p[i].v
		segL.Set(idx, idx)
		segR.Set(idx, idx)
	}
	out(ans)
