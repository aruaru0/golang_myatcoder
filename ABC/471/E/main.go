...
func main() {
	...
	for _, v := range a {
		vMod := v % mod.mod
		sumA = mod.add(sumA, vMod)
		sumA2 = mod.add(sumA2, mod.mul(vMod, vMod))
	}

	sumCross := mod.sub(mod.mul(sumA, sumA), sumA2)

	term1 := mod.mul(mod.nCr(N-1, K-1), sumA2)

	term2 := 0
	if K >= 2 {
		term2 = mod.mul(mod.nCr(N-2, K-2), sumCross)
	}

	ans := mod.add(term1, term2)
	out(ans)
}
