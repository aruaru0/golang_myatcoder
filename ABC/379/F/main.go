...
func lowerBoundGT(a []int, x int) int {
	idx := sort.Search(len(a), func(i int) bool {
		return a[i] <= x
	})
	return idx
...
func main() {
	...
	for i := n - 1; i >= 0; i-- {
		for _, e := range qs[i] {
			l, qi := e.a, e.b
			max_h := seg.Prod(l+1, i+1)    // [l+1, r]までの区間の最大の高さ
			now := lowerBoundGT(st, max_h) // max_hより高い建物の場所を取り出す
			ans[qi] = now                  // 数を代入
		}
		// h[i]より低いビルをPopする
