...
func main() {
	...
	for i := 0; i < N; i++ {
		...
		a[i] /= g
		b[i] /= g
		if a[i] == 0 { // a=0のときは0,1に置き換え
			x := m[pair{0, 1}]
			x.f++
			m[pair{0, 1}] = x
		} else if b[i] == 0 { // b=0のときは0,1に置き換え
			x := m[pair{0, 1}]
			x.s++
			m[pair{0, 1}] = x
		} else if a[i] > 0 && b[i] > 0 { // aもbも正
			x := m[pair{a[i], b[i]}]
			x.f++
			m[pair{a[i], b[i]}] = x
		} else if a[i] < 0 && b[i] < 0 { // aもbも負なら正に置き換え
			a[i] = -a[i]
			b[i] = -b[i]
			x := m[pair{a[i], b[i]}]
			x.f++
			m[pair{a[i], b[i]}] = x
		} else if a[i] < 0 { // aが正なら反転
			a[i] = -a[i]
			b[i] = -b[i]
			x := m[pair{-b[i], a[i]}]
			x.s++
