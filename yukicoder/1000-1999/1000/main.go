...
func main() {
	...
	var merger = func(a, b S) S {
		return S{a.a + b.a}
	}
	var mapper = func(f F, x S) S {
		return S{x.a + f.a}
	}
	var comp = func(f, g F) F {
		return F{f.a + g.a}
	}
	var id = func() F {
		return F{0}
	...
	seg := newLazySegtree(c, e, merger, mapper, comp, id)

	for i := 0; i < Q; i++ {
		c, x, y := getS(), getI(), getI()
		if c == "A" {
			x--
			a[x] += y
			b[x] -= seg.Get(x).a * y
			...
			x--
			seg.RangeApply(x, y, F{1})
			// for j := 0; j < N; j++ {
			// 	fmt.Fprint(wr, seg.Get(j))
			// }
			// out()
		}
	}
	for i := 0; i < N; i++ {
		val := b[i] + seg.Get(i).a*a[i]
		fmt.Fprint(wr, val, " ")
	}
	out()
}
