...
func initFrac() {
	...
	ifact[size-1] = mpow(fact[size-1], MOD-2)
	for i := size - 2; i >= 0; i-- {
...
func main() {
	...
	for i := 0; i < N; i++ {
		...
		powp[i][0] = 1
		for j := 0; j < K; j++ {
			powp[i][j+1] = mod.mul(powp[i][j], p[i])
		}
	}
	initFrac()
		...
		for j := 0; j < M+1; j++ {
			for k := 0; k < K+1; k++ {
				...
				for d := 1; k+d <= K; d++ {
					nj := j + 1
					nk := k + d
					now := mod.mul(powp[i][d], ifact[d])
					dp[ni][nj][nk] += dp[i][j][k] * now % mod.mod
					dp[ni][nj][nk] %= mod.mod
				}
			}
		}
