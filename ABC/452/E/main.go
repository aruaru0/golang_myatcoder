...
func main() {
	...
	for i := 1; i <= N; i++ {
		p[i] = p[i-1] + a[i-1]
		sum_a = mod.add(sum_a, mod.mul(i, a[i-1]))
	}
	...
	s1 := mod.mul(sum_a, sum_b)
	s2 := 0
	limit := min(N, M)

	for j := 1; j <= limit; j++ {
		tj := 0
		for start := j; start <= N; start += j {
			k := start / j
			end := min(N, start+j-1)
			sum_range := mod.sub(p[end], p[start-1])
			tj += k * sum_range
		}
		term_s2 := mod.mul(b[j-1], mod.mul(j, tj))
		s2 = mod.add(s2, term_s2)
	}
