...
func main() {
	...
	t := make([]*LazySegtree, 3)
	t[0] = newLazySegtree(make([]S, n), e, merger, mapper, comp, id)
	t[1] = newLazySegtree(make([]S, n), e, merger, mapper, comp, id)
	t[2] = newLazySegtree(make([]S, n), e, merger, mapper, comp, id)
	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < 3; j++ {
			t[j].RangeApply(pre[i]+1, i+1, F{1})
		}
	}
	ans := t[2].AllProd()
	out(ans.v)
