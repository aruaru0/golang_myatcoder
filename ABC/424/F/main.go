...
func main() {
	...
	e_max := func() int { return -inf }
	e_min := func() int { return inf }
	merger_max := func(a, b int) int {
		return max(a, b)
	}
	merger_min := func(a, b int) int {
		return min(a, b)
	}
	...
	seg_min := newSegtree(v_min, e_min, merger_min)
	seg_max := newSegtree(v_max, e_max, merger_max)
	...
	for i := 0; i < Q; i++ {
		...
		// 交わる条件は、
		// seg_maxをcが始点(c)の線分の終点(d)の値の最大値
		// seg_minをdが終点(d)の線分の始点(c)の値の最小値
		// とすると、
		// a < c < b < d -> seg_max(a, b) < b
		// または、
		// c < a < d < b -> a < seg_min(a, b)
