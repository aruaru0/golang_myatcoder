...
func main() {
	...
	for i := 0; i < N; i++ {
		for j := i; j < N; j++ {
			if i == j {
				x[i][j] = mod.add(x[i][j], 1)
			}
			if edge[i][j] {
				m2 := mod.div(1, 2*M)
				x[i][i] = mod.sub(x[i][i], m2)
				x[j][j] = mod.sub(x[j][j], m2)
				x[i][j] = mod.add(x[i][j], m2)
				x[j][i] = mod.add(x[j][i], m2)
			}
		}
	}

	x = mod.powModMatrix(x, K)
	ans := make([]int, N)
	for i := 0; i < N; i++ {
		for j := 0; j < N; j++ {
			ans[i] = mod.add(ans[i], mod.mul(x[i][j], a[j]))
		}
	}
	for i := 0; i < N; i++ {
		out(ans[i])
