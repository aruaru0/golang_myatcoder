...
func main() {
	...
	node2 := make([][]int, N)
	for i := 0; i < N-1; i++ {
		a, b, c, d := getI()-1, getI()-1, getI()-1, getI()
	...
	q := make([]query, 0)
	vq := make([][]int, N)   // query番号とノードの対応
	vlca := make([][]int, N) // query番号とlcaの対応
	for i := 0; i < Q; i++ {
		x, y, u, v := getI()-1, getI(), getI()-1, getI()-1
		lca := g.lca(u, v)
		q = append(q, query{x, y, u, v, lca, 0})
		vq[u] = append(vq[u], i)
		vq[v] = append(vq[v], i)
		vlca[lca] = append(vlca[lca], i)
	}

	dist := 0
	cdist := make([]int, N-1) // color distance
	cnum := make([]int, N-1)  // num of color
	var dfs func(v, parent int)
	dfs = func(v, parent int) {
		for _, qq := range vq[v] { // node v's query
			q[qq].ans += dist - cdist[q[qq].x] + cnum[q[qq].x]*q[qq].y // dist - colordist + y * numofcolor
		}
		for _, qq := range vlca[v] { // node v's lca
			q[qq].ans -= 2 * (dist - cdist[q[qq].x] + cnum[q[qq].x]*q[qq].y)
		}
		// 以下がDFSの部分
		for _, edge := range node[v] {
			if edge.to == parent {
				continue
			}
			cnum[edge.c]++
