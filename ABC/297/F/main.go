...
func main() {
	...
	for i := 1; i <= H; i++ {
		for j := 1; j <= W; j++ {
			...
			i1, i2 := i-1, max(i-2, 0)
			j1, j2 := j-1, max(j-2, 0)
			now = mod.sub(now, mod.nCr(i1*j, K)*2%mod.mod)
			now = mod.add(now, mod.nCr(i2*j, K))

			now = mod.sub(now, mod.nCr(i*j1, K)*2%mod.mod)
			now = mod.add(now, mod.nCr(i*j2, K))

			now = mod.add(now, mod.nCr(i1*j1, K)*4)
			now = mod.sub(now, mod.nCr(i1*j2, K)*2)
			now = mod.sub(now, mod.nCr(i2*j1, K)*2)
			now = mod.add(now, mod.nCr(i2*j2, K))

			tmp := mod.mul(now, H-i+1)
			tmp = mod.mul(tmp, (W - j + 1))
			tmp = mod.mul(tmp, mod.mul(i, j))
			ans = mod.add(ans, tmp)

		}
	}

	ans = mod.div(ans, mod.nCr(H*W, K))
	out(ans)
