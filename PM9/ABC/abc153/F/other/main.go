...
func main() {
	...
	N, D, A := getI(), getI(), getI()
	m := make([]monster, N)
	for i := 0; i < N; i++ {
		m[i] = monster{getI(), getI()}
	}
	...
	seg := newLazySegtree(h, e, merger, mapper, comp, id)

	ans := 0
	for i := 0; i < N; i++ {
		hp := seg.Prod(i, i+1).hp
		if hp <= 0 {
			continue
		}
		cnt := (hp + A - 1) / A
		rng := upperBound(x, x[i]+2*D)
		seg.RangeApply(i, rng, F{-A * cnt})
		// out(hp, cnt, rng, x)
		ans += cnt
	}
	out(ans)
