...
var mod = newModint(1e9 + 7)

var c [310][310]int

func f(n, m, l int) int {
	path := make([]int, l+3)
	path[1] = 1
	path[2] = 1
	for i := 3; i <= l; i++ {
		path[i] = mod.mul(path[i-1], i)
	}
	cycle := make([]int, l+1)
	for i := 2; i <= l; i++ {
		cycle[i] = path[i-1]
	...
	for i := 0; i <= n; i++ {
		for j := 0; j <= m; j++ {
			for k := 1; k <= l; k++ {
				...
				x := mod.mul(c[n-i-1][k-1], path[k])
				y := mod.mul(dp[i][j], x)
				dp[ni][nj] = mod.add(dp[ni][nj], y)
			}
			for k := 2; k <= l; k++ {
				ni, nj := i+k, j+k
				if ni > n {
...
func main() {
	...
	for i := 0; i < 305; i++ {
		for j := 0; j <= i; j++ {
			c[i+1][j] = mod.add(c[i+1][j], c[i][j])
			c[i+1][j+1] = mod.add(c[i+1][j+1], c[i][j])
		}
	}

	N, M, L := getI(), getI(), getI()
	ans := mod.sub(f(N, M, L), f(N, M, L-1))
	out(ans)
}
