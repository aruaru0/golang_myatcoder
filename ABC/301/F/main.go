...
func inits(N int) {
	...
	for i := 0; i < N; i++ {
		fact[i+1] = fact[i] * (i + 1) % mod.mod
		p26[i+1] = p26[i] * 26 % mod.mod
	}
	ifact[N] = mod.div(1, fact[N])
	for i := N; i > 0; i-- {
...
func main() {
	...
	dp[0][0] = 1
	for i := 0; i < N; i++ {
		dp[i+1][0] += dp[i][0] * lower(S[i]) % mod.mod
		dp[i+1][0] %= mod.mod
		dp[i+1][1] += dp[i][0] * upper(S[i]) % mod.mod
		dp[i+1][1] %= mod.mod
		dp[i+1][1] += dp[i][1] * upper(S[i]) % mod.mod
		dp[i+1][1] %= mod.mod
	}
	...
	for i := N - 1; i >= 0; i-- {
		sub := 0
		u := len(s)
		for v := 0; v <= min(26-u, q); v++ {
			tmp := binom(q, v) * fact[26-u] % mod.mod
			tmp = tmp * ifact[26-u-v] % mod.mod
			tmp = tmp * p26[q-v] % mod.mod
			sub = (sub + tmp) % mod.mod
		}
		ans += sub * lower(S[i]) % mod.mod * dp[i][1] % mod.mod
		ans %= mod.mod
		if S[i] == '?' {
			q++
