...
func main() {
	...
	mapper := func(f F, x S) S {
		return S{
			...
			(x.b + x.c*f.b%mod) % mod,
			x.c,
			(x.s + x.a*f.b%mod + x.b*f.a%mod + x.c*f.a%mod*f.b%mod) % mod,
		}
	}
	comp := func(f, g F) F {
		return F{(f.a + g.a) % mod, (f.b + g.b) % mod}
	}
	id := func() F {
		return F{0, 0}
	}
	...
	for q := 0; q < Q; q++ {
		...
		if t == 1 {
			...
			seg.RangeApply(l, r, F{x, 0})
		} else if t == 2 {
			l, r, x := getI()-1, getI(), getI()
			seg.RangeApply(l, r, F{0, x})
		} else {
			l, r := getI()-1, getI()
			s := seg.Prod(l, r)
			out(s.s)
