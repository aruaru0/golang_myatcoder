...
func main() {
	...
	mapper := func(f F, x S) S {
		return S{min(f.a, x.a)}
	}
	comp := func(f, g F) F {
		return F{min(f.a, g.a)}
	}
	id := func() F {
		return F{inf}
	}
	...
	xseg := newLazySegtree(x, e, merger, mapper, comp, id)
	yseg := newLazySegtree(x, e, merger, mapper, comp, id)

	// for i := 0; i < N; i++ {
	// 	out(xseg.Get(i), yseg.Get(i))
	// }
	...
	for i := 0; i < Q; i++ {
		...
		if q == 1 {
			x := xseg.Get(pos)
			yseg.RangeApply(0, x.a, F{pos})
			// out(x.a)
			cnt += x.a - 1
		} else {
			y := yseg.Get(pos)
			xseg.RangeApply(0, y.a, F{pos})
			// out(y.a)
			cnt += y.a - 1
		}
	}
