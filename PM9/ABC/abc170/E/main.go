...
func main() {
	...
	pq := make([]priorityQueue, 210000) // 各園の優先キュー
	for i := 0; i < N; i++ {
		heap.Push(&pq[b[i]], pqi{a[i], i}) // 初期位置に配置
	}
	...
	seg := newSegtree(v, e, merger) // セグメントツリー（min）

	for i := 0; i < Q; i++ {
		c, d := getI()-1, getI()-1 // 園児CをDに移動させる
		pos := b[c]                // 現在の園児の位置
		b[c] = d                   // 園児を移動させる

		// 移動元の処理
		for b[pq[pos][0].idx] != pos { // 現在位置のレート最大の園児が既に移動していれば
			heap.Pop(&pq[pos]) // キューから削除
			if len(pq[pos]) == 0 {
				break
			}
		}
		if len(pq[pos]) == 0 {
			...
			seg.Set(pos, pq[pos][0].a)
		}

		// 移動先の処理
		heap.Push(&pq[d], pqi{a[c], c})
		for b[pq[d][0].idx] != d {
			heap.Pop(&pq[d])
		}
		seg.Set(d, pq[d][0].a)
		out(seg.Prod(0, 210000))
	}
}
