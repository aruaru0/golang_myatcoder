...
func main() {
	...
	for i := 0; i < N-1; i++ {
		...
		pos := len(node)
		node = append(node, make([]int, 0))
		node[pos] = append(node[pos], nodeNum[p])
		node[pos] = append(node[pos], nodeNum[q])
		uf.Merge(p, q)
		nodeNum[uf.Leader(p)] = pos
		size[pos] = uf.Size(p)
	}

	root := len(node) - 1
	dp := make([]int, len(node))
	var dfs func(cur int)
	dfs = func(cur int) {
		for _, e := range node[cur] {
			dp[e] = mod.add(dp[cur], mod.div(size[e], size[cur]))
			dfs(e)
		}
	}
