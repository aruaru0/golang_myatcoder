...
func main() {
	...
	merger := func(a, b S) S {
		return S{a.s0 + b.s0, a.s1 + b.s1, a.s2 + b.s2}
	}
	mapper := func(f F, x S) S {
		x.s2 += mod.mul(x.s1, mod.mul(f.v, 2))
		x.s2 += mod.mul(x.s0, mod.mul(f.v, f.v))
		x.s2 %= mod.mod
		x.s1 += mod.mul(x.s0, f.v)
		x.s1 %= mod.mod
		return x
	}
	comp := func(f, g F) F {
		return F{mod.add(f.v, g.v)}
	}
	...
	for qi := 0; qi < Q; qi++ {
		l, r, a := getI()-1, getI(), getI()
		seg.RangeApply(l, r, F{a})
		s := seg.Prod(l, r)
		ans := mod.sub(mod.mul(s.s1, s.s1), s.s2)
		ans = mod.div(ans, 2)
		out(ans)
	}
}
