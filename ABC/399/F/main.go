...
func main() {
	...
	pow_x := make([][]int, n+1)
	for i := 0; i <= n; i++ {
		pow_x[i] = make([]int, k+1)
		for j := 0; j <= k; j++ {
			pow_x[i][j] = mod.pow(x[i], j)
		}
	}

	// T[k][i] = sum_{j=0..i} of x[j]^k の累積をとる (i=0..N)
	//    k=0..K
	T := make([][]int, k+1)
	for i := 0; i <= k; i++ {
		T[i] = make([]int, n+1)
	...
	for r := 1; r <= n; r++ {
		for j := 0; j <= k; j++ {
			...
			addVal := mod.mul(c, sign)
			addVal = mod.mul(addVal, val_r)
			addVal = mod.mul(addVal, val_t)
			ans += addVal
			ans %= mod.mod
		}
	}
