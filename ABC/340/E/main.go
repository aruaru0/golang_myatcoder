...
func main() {
	...
	for i := 0; i < M; i++ {
		...
		cnt, diff := n/N, n%N
		c := 1
		l, r := (c+b[i])%N, (c+b[i]+diff)%N
		// out(n, "cnt, diff", cnt, diff, "l, r = ", l, r)
		if l > r {
			seg.RangeApply(l, N, F{1})
			seg.RangeApply(0, r, F{1})
		} else {
			seg.RangeApply(l, r, F{1})
		}
		seg.RangeApply(0, N, F{cnt})
		// for j := 0; j < N; j++ {
		// 	fmt.Fprint(wr, seg.Get(j).a, " ")
		// }
		// out()
	}

	for i := 0; i < N; i++ {
		fmt.Fprint(wr, seg.Get(i).a, " ")
	}
	out()
}
