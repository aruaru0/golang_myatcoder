...
func main() {
	...
	S %= mod.mod
	sum := 0
	sum += mod.div((S-1)*(S-2)%mod.mod*(S-3)%mod.mod*(S-4)%mod.mod*(S-5)%mod.mod, 120)
	sum %= mod.mod
	sum += 6 * (((S-1)*t1%mod.mod - 2*t1%mod.mod*(t1+1)%mod.mod + mod.mod) % mod.mod) % mod.mod
	sum %= mod.mod
	sum += 3 * (((1-S+mod.mod)%mod.mod*t2%mod.mod-mod.div(t2*(t2+1)%mod.mod*(2*t2%mod.mod+1)%mod.mod, 3)+mod.mod)%mod.mod + mod.div((S+1)%mod.mod*t2%mod.mod*(t2+1)%mod.mod, 2))
	sum %= mod.mod
	if T%3 == 0 {
		sum += 8 * (mod.div(S, 3) - 1) % mod.mod
		sum %= mod.mod
	}
	if T%2 == 0 {
		sum += mod.div(6*(mod.div(S, 2)-1)%mod.mod*(mod.div(S, 2)-2)%mod.mod, 2)
		sum %= mod.mod
	}
	fmt.Println(mod.div(sum, 24))
}
