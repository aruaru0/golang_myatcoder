...
func main() {
	...
	e := func() S { return S{minit(0), minit(1)} }
	merger := func(a, b S) S {
		return S{
			plus(a.h, mul(b.h, a.c)),
			mul(a.c, b.c)}
		// a.h + b.h*a.c, a.c * b.c}
	}
	...
	seg1 := newSegtree(v1, e, merger)
	seg2 := newSegtree(v2, e, merger)

	for q := 0; q < Q; q++ {
		op := getI()
		if op == 1 {
			i, c := getI()-1, int(getS()[0])
			seg1.Set(i, S{minit(c), minit(x)})
			seg2.Set(N-1-i, S{minit(c), minit(x)})
		} else {
			l, r := getI()-1, getI()
			h1 := seg1.Prod(l, r).h
			h2 := seg2.Prod(N-r, N-l).h
			if h1 == h2 {
				out("Yes")
			} else {
				out("No")
