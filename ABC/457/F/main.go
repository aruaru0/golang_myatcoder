...
func main() {
	...
	merger := func(a, b S) S {
		return S{(a.val + b.val) % mint}
	}
	mapper := func(f F, x S) S {
		return S{(x.val * f.val) % mint}
	}
	comp := func(f, g F) F {
		return F{(f.val * g.val) % mint}
	}
	id := func() F {
		return F{1}
	}

	seg := newLazySegtree(make([]S, n), e, merger, mapper, comp, id)
	...
	for i := n - 3; i >= 0; i-- {
		x := seg.Get(i + d[i])
		if d[i] != d[i+1] {
			seg.RangeApply(0, n, F{0})
		} else {
			seg.RangeApply(0, n, F{n - i - 2})
		}
		seg.Set(i, x)
		seg.Set(i+d[i], S{seg.Get(i+d[i]).val + x.val})
	}
