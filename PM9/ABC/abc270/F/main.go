...
func main() {
	...
	for bit := 0; bit < 4; bit++ {
		...
		e := make([]edge, 0)
		for i := 0; i < M; i++ {
			e = append(e, edge{a[i] - 1, b[i] - 1, z[i]})
		}
		if bit%2 == 1 {
			for i := 0; i < N; i++ {
				e = append(e, edge{i, n, x[i]})
			}
			n++
		}
		if (bit>>1)%2 == 1 {
			for i := 0; i < N; i++ {
				e = append(e, edge{i, n, y[i]})
			}
			n++
		}
		...
		for _, v := range e {
			...
			if uf.Same(v.from, v.to) == false {
				cost += v.cost
				uf.Merge(v.from, v.to)
			}
		}
		if uf.Size(0) == n {
			ans = min(ans, cost)
