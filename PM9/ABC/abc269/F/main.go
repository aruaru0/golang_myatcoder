...
func f(x, y, a int) int {
	// 矩形の和＝対角の和*x*y/2
	// mean = a+(a+ 2m*(x-1) + 2*(y-1))
	// mean*x*y/2
	a = mod.add(a, a)
	b := mod.mul(mod.mul(mod.sub(x, 1), M), 2)
	c := mod.mul(mod.sub(y, 1), 2)
	res := mod.add(a, mod.add(b, c))
	res = mod.mul(res, mod.mul(x, mod.div(y, 2)))
	return res
}
...
func calc(x, y int) int {
	res := 0
	// 奇数（行、列）
	res = mod.add(res, f((x+1)/2, (y+1)/2, 1))
	// 偶数（行、列）
	res = mod.add(res, f(x/2, y/2, M+2))
	return res
}
...
func main() {
	...
	Q := getI()
	for i := 0; i < Q; i++ {
		a, b, c, d := getI()-1, getI(), getI()-1, getI()
		ans := calc(b, d)
		ans = mod.sub(ans, calc(a, d))
		ans = mod.sub(ans, calc(b, c))
		ans = mod.add(ans, calc(a, c))
		out(ans)
	}
}
