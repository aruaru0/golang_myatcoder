...
func main() {
	...
	bit := newBIT(2100)
	for i := 0; i < N; i++ {
		inner = mod.add(inner, bit.rangeSum(a[i]+1, 2100))
		bit.add(a[i], 1)
	}
	outer := 0
	for i := 0; i < N; i++ {
		outer = mod.add(outer, bit.rangeSum(a[i]+1, 2100))
	}
	// out(inner, outer)
	// out(K)
	ans := mod.mul(K-1, K)
	ans = mod.div(ans, 2)
	// out(ans)
	ans = mod.mul(ans, outer)
	ans = mod.add(ans, mod.mul(inner, K))
	out(ans)
