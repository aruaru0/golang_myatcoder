...
func main() {
	...
	e := func() S { return S{inf} }
	merger := func(a, b S) S { return S{min(a.v, b.v)} }
	mapper := func(f F, x S) S { return S{f.x + x.v} }
	comp := func(f, g F) F { return F{f.x + g.x} }
	...
	for i := 0; i < Q; i++ {
		t, l, r := getI(), getI(), getI()
		if t == 1 {
			if s[l-1] != s[r-1] {
				if s[l-1] == '(' {
					seg.RangeApply(l, N+1, F{-2})
					seg.RangeApply(r, N+1, F{2})
				} else {
					seg.RangeApply(l, N+1, F{2})
					seg.RangeApply(r, N+1, F{-2})
				}
			...
			// seg.Prod(l-1, r+1) == seg.Get(l-1) は、範囲内でマイナスになっていない条件
			// seg.Get(l-r) == seg.Get(r)は、前後が同じ条件
			if seg.Prod(l-1, r+1) == seg.Get(l-1) && seg.Get(l-1) == seg.Get(r) {
				out("Yes")
			} else {
				out("No")
			}
