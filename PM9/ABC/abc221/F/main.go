...
// sv-tvへの経路を求める
func dfs2(cur, prev, tv int) bool {
	if cur == tv {
		vs = append(vs, cur)
		return true
	}
...
func main() {
	...
	ans := 0
	if d%2 == 1 {
		// 経路の長さが奇数の場合は、中心が辺にあるので
		// 中心から左右にd/2の経路を求めて、それらをかけ合わせる
		l, r := vs[d/2], vs[d/2+1]
		ans = dfs3(l, r, d/2)
		ans = mod.mul(ans, dfs3(r, l, d/2))
	} else { // 経路が偶数の場合
		// 中心が点にある場合は、その点を通過する経路を足し合わせる
		ans = 1
		c := vs[d/2]
		s1 := 0
