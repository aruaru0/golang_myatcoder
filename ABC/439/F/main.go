...
func main() {
	...
	for i := 0; i < N; i++ {
		term1 := mod.mul(L[i], R[i])
		ans = mod.add(ans, term1)

		if i > 0 {
			term2 := mod.mul(R[i], mod.pow(2, i-1))
			term2 = mod.mul(term2, currentSumL)
			ans = mod.add(ans, term2)
		}

		addVal := mod.mul(L[i], mod.div(1, mod.pow(2, i)))
		currentSumL = mod.add(currentSumL, addVal)
	}
