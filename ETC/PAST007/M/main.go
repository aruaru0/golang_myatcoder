...
func main() {
	...
	mf := newMinCostFlow(2*N + 2)
	mf.AddEdge(start, goal, N, C)
	for i := 0; i < N; i++ {
		mf.AddEdge(start, from+i, 1, 0)
	}
	for i := 0; i < N; i++ {
		mf.AddEdge(to+i, goal, 1, 0)
	}
	for i := 0; i < N; i++ {
		for j := i + 1; j < N; j++ {
			mf.AddEdge(from+i, to+j, 1, abs(a[i]-a[j]))
		}
	}
	ret := mf.FlowL(start, goal, N)
	out(ret[1])
}
